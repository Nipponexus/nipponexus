...
【出力形式】まず日本語本文(2,400字以上)、次に区切り線 ===EN=== 、続けて英語本文(日本語の2倍以上かつ2,400字以上)。英語本文に半角ダブルクォートを使わない。前置き・後書き・メタ発言は書かず本文のみ出力。"""
...
def verify(ja, en):
    ...
    r["ok"] = (r["ja_len"]>=2400 and r["en_len"]>=max(2400, r["ja_len"]*2)
               and r["h"]>=6 and r["b"]>=6)
    return r
